/**
 * Сервис для работы с BI API.
 * Зависимости передаются через конструктор (config с Moniker, ClientServiceUrl).
 */
import { CommonMessages } from '../Common/constants.js';

export function BiService(config) {
	this.config = config || {};
	this._moniker = this.config.Moniker;
	this._clientServiceUrl = this.config.ClientServiceUrl;
}

BiService.prototype.ItDataType = {
	NoData: 0,
	String: 1,
	Integer: 2,
	Float: 3,
...
BiService.prototype.OpenArgs = function (id, value, type) {
	return {
		dt: type,
		id: id,
		n: id,
...
BiService.prototype.getStressId = async function (ctx) {
...
BiService.prototype.getResultForeModule = async function (ctx) {
	debugLog("getResultForeModule");
	let rez = "";
	let _params = {
		"ForeExec": {
...
BiService.prototype.openRds = async function (ctx) {
	debugLog("openRds");
	let rez = "";
	let _params = {
		"OpenRds": {
...
BiService.prototype.closeRds = async function (ctx) {
	debugLog("closeRds");
	let rez = "";
	let _params = {
		"CloseRds": {
...
BiService.prototype.getRdsElements = async function (ctx) {
...
BiService.prototype.GetStatus = async function (ctx) {
	debugLog("GetStatus");
	let rez = "";
	let _params = {
		"GetStatus": {
...
BiService.prototype.GetMbSec = async function (ctx) {
...
BiService.prototype.openDim = async function (ctx) {
	debugLog("openDim");
	let rez = "";
	let _params = {
		"OpenDim": {
...
BiService.prototype.closeDim = async function (ctx) {
	debugLog("closeDim");
	let rez = "";
	let _params = {
		"CloseDim": {
...
BiService.prototype.clearCache = async function (ctx) {
	debugLog("clearCache");
	let rez = "";
	let _params = {
		"GetObjects": {
...
BiService.prototype.getFiltredDimElements = async function (ctx) {
	let rez = "";
	let _params = {
		"GetDimElements": {
			"tDim": { "id": `${ctx.key}` },
...
BiService.prototype.openQuery = async function (ctx) {
	let rez = "";
	let _params = {
		"GetQuery": {
			"tOb": { "id": `${this._moniker}!${ctx.rdsKey}` },
...
BiService.prototype.updateFromConfig = function (config) {
	if (config) {
		this.config = config;
